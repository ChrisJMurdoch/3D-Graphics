...
void OBJModel::setMaterial(std::string &name)
{
	int index;
	if (materials.find(name) == materials.end()) { // Insert
		index = materials.size();
		materials[name] = index;
		materialMapping[index] = name;
	} else { // Assign
...
void OBJModel::writeMaterial(std::string &token)
{
	int texIndex = std::stoi(SplitString(token, '/')[0]) - 1;
	if (texIndices.size()<=texIndex) {
		texIndices.resize(texIndex+10000);
	}
	texIndices[texIndex] = material;
}
	...
	if (file.is_open())
	{
		while (file.good())
		{
			...
			case 'v':
				...
				break;
			case 'u': // usemtl
				setMaterial( SplitString(line, ' ')[1] );
				break;
			case 'f':
				CreateOBJFace(line);
				break;
...
IndexedModel OBJModel::ToIndexedModel()
{
	...
	for (unsigned int i = 0; i < numIndices; i++)
	{
		...
		unsigned int r = result.texIndices.size() % 13;
		float currentTexIndex = texIndices[currentIndex->vertexIndex];
		...
		if (it == normalModelIndexMap.end())
		{
			...
			normalModel.texIndices.push_back(currentTexIndex);
		...
		if (previousVertexLocation == (unsigned int)-1)
		{
			...
			result.texIndices.push_back(currentTexIndex);
